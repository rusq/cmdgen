...
Args:
	for i, arg := range args {
		...
		helpSuccess := "$$Command$$ help"
		if i > 0 {
			helpSuccess += " " + strings.Join(args[:i], " ")
		}
		fmt.Fprintf(os.Stderr, "$$Command$$ help %s: unknown help topic. Run '%s'.\n", strings.Join(args, " "), helpSuccess)
		base.SetExitStatus(2) // failed at '$$Command$$ help cmd'
		base.Exit()
	}
	...
	// not exit 2: succeeded at '$$Command$$ help cmd'.
}

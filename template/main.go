...
func main() {
	if isRoot() {
		log.Fatal("$$Command$$:  cowardly refusing to run as root")
	}
...
BigCmdLoop:
	for bigCmd := base.MAIN__COMMAND; ; {
		...
		fmt.Fprintf(os.Stderr, "$$Command$$ %s: unknown command\nRun '$$Command$$ help%s' for usage.\n", base.CmdName, helpArg)
...
func mainUsage() {
	help.PrintUsage(os.Stderr, base.MAIN__COMMAND)
	os.Exit(2)
}
